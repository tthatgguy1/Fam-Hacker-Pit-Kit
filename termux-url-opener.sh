...
deluxe_cowsay "Welcome To Fam-Hacker Tube Client. You Can Download All Sorts Of Stuff Here. Right my script is gathering metadata needed for a download. Please Hold Tight"
...
1)
  ...
  printf '%b\n' "         ${VBLU}┌────────────────────────────────────────────────┐${RESET}"
  printf '%b\n' "  ${VBLU}┌──────┤ ${ACID}[${BMGT}Running${ACID}]${BMGT}:  ${BCYN}──${ACID}> ${NGRN}Highest Quality Video Download ${VBLU}├─────┐${RESET}"
  printf '%b\n' "  ${VBLU}│      └────────────────────────────────────────────────┘   │${RESET}"
  sleep 1.0
  ...
  if [[ "$rc" -eq 0 ]]; then
    ...
    printf '\n\n'
    deluxe_cowsay "Your MKV File Has Failed To Download"
  ...
  if [[ -f "$NF_PRODUCT" ]]; then
    ...
    printf '\n\n'
    deluxe_cowsay "${NONG}Transfer Failed: ${NF_PRODUCT} not found${RESET}"
    mpv "$HOME/storage/shared/Fam-Hacker-Pit-Kit/.NFS-Data/sfx/MFGTA.mp3" >/dev/null 2>&1
...
2)
  ...
  if [[ "$rc" -eq 0 ]]; then
    ...
    mpv "$HOME/storage/shared/Fam-Hacker-Pit-Kit/.NFS-Data/sfx/FHS.mp3" >/dev/null 2>&1
    printf '\n\n'
    deluxe_cowsay "Your MKV File Has Downloaded Successfully" | lolcat
    mpv "$HOME/storage/shared/Fam-Hacker-Pit-Kit/.NFS-Data/sfx/TXS.mp3" >/dev/null 2>&1
    sleep 1.0
    clear
  else
    printf '\n\n'
    deluxe_cowsay "2K File Has Failed To Download"
    mpv "$HOME/storage/shared/Fam-Hacker-Pit-Kit/.NFS-Data/sfx/MFGTA.mp3" >/dev/null 2>&1
    printf '%b\n' "${VBLU}┌──${BCYN}Press${VBLU}─${BCYN}Any${VBLU}─${BCYN}Button${VBLU}─${BCYN}To${VBLU}─${BCYN}Move${VBLU}─${BCYN}On${VBLU}──┐${RESET}"
    printf '%b'   "${VBLU}└──${BMGT}>${ACID}: ${RESET}"
    read -r
