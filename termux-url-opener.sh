...
OPT_BMKV_CMD=(
  ...
  -o "$OUT_TEMPLATE_BEST"
    ...
    --no-overwrites
    --progress-template "download:    ${VBLU}└──${ACID}>  ${BCYN}DL${BMGT}: %(progress._percent_str)s ${VBLU}── ${BCYN}ETA${BMGT}: %(progress._eta_str)s ${VBLU}── ${BCYN}Speed${BMGT}:%(progress._speed_str)s    ${ACID}<${VBLU}──┘${RESET}"
  "$URL"
)
...
2)
  ...
  printf '%b\n' "         ${VBLU}┌───────────────────────────────────────────────┐${RESET}"
  printf '%b\n' "    ${VBLU}┌────┤ ${NONG}[${BWHT}Running${NONG}]${NGRN}: ${BCYN}──${BMGT}>  ${ACID}UHD 2K Quality Video Download ${VBLU}├─────┐${RESET}"
  printf '%b\n' "    ${VBLU}│    └───────────────────────────────────────────────┘     │${RESET}"
  sleep 1.0
