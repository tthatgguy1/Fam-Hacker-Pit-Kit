...
DEFAULT="1"
clear
toiket -f smslant "DL Options" -F gay
printf "\n\n"
printf "   ${VBLU}┌───────────────────────────────────────────────────────────┐\n"
...
printf "    ${VBLU}├───────────────────────────────────────────────────────────────┘\n"
printf "    ${VBLU}│  ${NONG}───${BWHT}>  📺   ${ACID}Chl${NONG}:    [%s]\n"     "$CHANNEL"
printf "    ${VBLU}│  ${NONG}───${BWHT}>  🙋   ${ACID}Sbs${NONG}:    [%s]\n"     "$SUBS"
printf "    ${VBLU}│  ${NONG}───${BWHT}>  👍   ${ACID}Lks${NONG}:    [%s]\n"     "$LIKES"
printf "    ${VBLU}│  ${NONG}───${BWHT}>  📹   ${ACID}Res${NONG}:    [%sx%s]\n"  "$QLTYW" "$QLTYH"
printf "    ${VBLU}│  ${NONG}───${BWHT}>  📆   ${ACID}Upd${NONG}:    [%s]\n"     "$UPLOAD_DATE_FMT"
printf "    ${VBLU}└───────────────────────────────────────────────────────────────┘\n\n"
printf "           ${VBLU}┌───────────────────────────────────────────────┐${RESET}\n"
printf "    ${BCYN}<${VBLU}──────┤ ${BMGT}[${BCYN}Running${BMGT}]${BCYN}:${NONG}≡≡${BWHT}> ${BCYN}Highest ${VBLU}Quality ${BCYN}Video ${VBLU}Download  ├────────>\n"
printf "           ${VBLU}└───────────────────────────────────────────────┘${RESET}\n"
sleep 1.0
"${OPT_BMKV_CMD[@]}"
rc=$?
...
new_mkv=$(ls -t -- *.mkv 2>/dev/null | head -n1)
  if [ -n "${new_mkv}" ]; then
    mv -- "${new_mkv}" "${VIDEO_OUT}" >/dev/null 2>&1
